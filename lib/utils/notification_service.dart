  ...
  Future<void> _initializeFirebaseMessaging() async {
    try {
      ...
      if (Platform.isIOS) {
        ...
        // Diagnostic: Check if APNs token is available (required for FCM token on iOS)
        try {
          String? apnsToken = await _firebaseMessaging!.getAPNSToken();
          debugPrint('🍎 iOS APNs Token status: ${apnsToken != null ? "RECEIVED ✅" : "NULL ❌"}');
          if (apnsToken == null) {
            debugPrint('⚠️ Warning: APNs token is null. FCM token will likely fail to generate on iOS.');
            debugPrint('   Ensure you are using a PHYSICAL DEVICE and have APNs configured in Firebase Console.');
          } else {
