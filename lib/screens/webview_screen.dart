...
class _WebViewScreenState extends State<WebViewScreen> {
  ...
  /// Save FCM token to backend if access token is available
    ...
    try {
        ...
        (function() {
          ...
          window.fetch = async function(url, options) {
            ...
            var isLogin = urlString.includes('/auth/login') || 
                          ...
                          urlString.includes('/api/auth/verify-otp');
            ...
            try {
              ...
              if (isLogin) {
                 clone.json().then(data => {
                    if (data && (data.token || data.accessToken || (data.user && data.user.token))) {
      ...
      controller.addJavaScriptHandler(
          ...
          callback: (args) async {
            if (args.isNotEmpty) {
              try {
                final Map<String, dynamic> data = jsonDecode(args[0].toString());
                final body = data['body'] as Map<String, dynamic>?;

                if (body != null) {
                  // The user mentioned: { "token": "...", "user": { ... } }
                  final token = body['token'] ??
                      body['accessToken'] ??
                      body['access_token'] ??
                      (body['user'] is Map ? body['user']['token'] : null);

                  if (token != null && token.toString().isNotEmpty) {
                    final savedToken = PrefsUtil.getAccessToken();
                    if (savedToken != token.toString()) {
                      debugPrint('🔐 Captured Logged-in Token: ${token.toString().substring(0, 10)}...');
                      await PrefsUtil.setAccessToken(token.toString());
                      
                      // After getting token, immediately try to save FCM token
                      await _saveFCMToken();
                    }
                  }
                }
              } catch (e) {
                debugPrint('❌ Error processing captured login response: $e');
              }
            }
          });      
  ...
  Future<void> _checkLocalStorageForToken(
      InAppWebViewController controller) async {
    try {
      // Script to check for token in localStorage and cookies
      const script = r"""
        (function() {
          try {
            ...
            if (!token) {
               // Try checking user object in localStorage
               var user = localStorage.getItem('user');
               if (user) {
                 try {
                   var userObj = JSON.parse(user);
                   if (userObj.token) token = userObj.token;
                   if (userObj.accessToken) token = userObj.accessToken;
                 } catch(e) {}
               }
            }
            ...
            if (token) {
              if (window.flutter_inappwebview && window.flutter_inappwebview.callHandler) {
                window.flutter_inappwebview.callHandler('foundTokenInStorage', token);
              }
            }
          } catch(e) {
      ...
      controller.addJavaScriptHandler(
          handlerName: 'foundTokenInStorage',
          callback: (args) async {
            if (args.isNotEmpty) {
              final token = args[0].toString();
              if (token.isNotEmpty) {
                 final currentToken = PrefsUtil.getAccessToken();
                 if (currentToken != token) {
                    debugPrint('🔐 Found Access Token in Storage: ${token.substring(0, 15)}...');
      ...
      debugPrint('❌ Failed to check localStorage for token: $e');
    }
  }
                  ...
                  children: [
                    InAppWebView(
                      ...
                      onLoadStop: (controller, url) async {
                        ...
                        // await _checkLocalStorageForToken(controller);
                      },
